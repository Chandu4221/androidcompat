...
function isAlreadyVerified(combo, compatData) {
  return compatData.combinations.some(c =>
    c.agp === combo.agp &&
    c.ksp === combo.ksp &&
    c.hilt === combo.hilt &&
    c.status === 'verified'
          ...
          for (const gradle of gradleVersions) {
            ...
            for (const composeBom of composeBomVersions) {
              ...
              if (isAlreadyVerified(combo, compatData)) {
                console.log(`  ⏭️  Skipping already verified: AGP ${agp} + KSP ${ksp} + Hilt ${hilt}`);
                continue;
              }
...
function main() {
  ...
  const compatData = JSON.parse(fs.readFileSync(COMPAT_PATH, 'utf8'));
  ...
  candidates.forEach((c, i) => {
    console.log(`  ${i + 1}. AGP ${c.agp} | Kotlin ${c.kotlin} | KSP ${c.ksp} | Hilt ${c.hilt} | Gradle ${c.gradle}`);
  });
}
